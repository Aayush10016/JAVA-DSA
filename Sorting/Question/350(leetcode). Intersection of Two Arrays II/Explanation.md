...
…should appear `min(k, m)` times in the result.
...
# 🔍 Key Difference From Question 349
...
👉 You **must** track how many times each element appears.
...
This means:
- If a number appears 3 times in nums1 and 5 times in nums2 → output it 3 times  
- If it appears once in nums1 and zero times in nums2 → output it zero times  
...
# 🔹 Approach 1: HashMap Frequency Count (Most Recommended)
...
# 🔹 Approach 3: Frequency Array (If Range Is Small)

If values are within a small fixed range (like 0–1000):
...
### ❌ Returning unique intersection  
This belongs to Q349.

### ❌ Trying nested loops (`O(n²)`)  
Unacceptable for large inputs.

### ❌ Not decreasing HashMap count  
This causes too many duplicates.

### ❌ Forgetting order doesn’t matter  
